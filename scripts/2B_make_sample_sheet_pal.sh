...
find ${FASTQDIR_pal} -maxdepth 1 -name "*.fastq" | grep -e "_1\." | sort > R1.list
find ${FASTQDIR_pal} -maxdepth 1 -name "*.fastq" | grep -e "_2\." | sort > R2.list
...
do
    ...
    echo "${sample_line}" >> $SAMPLE_SHEET_PATH_pal
done
